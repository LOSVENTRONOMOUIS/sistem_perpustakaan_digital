...
<body class="text-slate-800 antialiased bg-slate-50 overflow-x-hidden">
    ...
    <aside id="sidebar" class="fixed top-0 left-0 h-screen w-72 bg-white shadow-xl z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 flex flex-col">
        ...
        <nav class="flex-1 px-4 overflow-y-auto space-y-2">
            ...
            <a href="anggota.php" class="flex items-center gap-3 px-4 py-3 bg-blue-600 text-white rounded-xl shadow-md shadow-blue-200 transition-all">
    ...
    <div class="lg:ml-72 transition-all duration-300 min-h-screen flex flex-col">
        ...
        <header class="h-20 bg-white shadow-sm flex items-center justify-between px-6 lg:px-10 z-30 sticky top-0 rounded-b-3xl">
            <div class="flex items-center gap-4">
                ...
                <h4 class="text-xl font-bold text-slate-800 hidden sm:block">Data Anggota</h4>
            </div>
        ...
        <main class="flex-1 p-6 lg:p-10">
            ...
            <div class="mb-8">
                ...
                <p class="text-slate-500 mt-1 text-base">Kelola data anggota perpustakaan digital</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white p-6 rounded-3xl shadow-sm hover:shadow-md hover:-translate-y-1 transition-all border border-slate-50">
                    <div class="w-16 h-16 rounded-2xl bg-blue-600 text-white flex items-center justify-center text-3xl mb-4 shadow-lg shadow-blue-200">
                        <i class="bi bi-people-fill"></i>
                    </div>
                    <h2 class="text-3xl font-bold text-slate-800"><?= isset($totalAnggota) ? $totalAnggota : 0 ?></h2>
                    <p class="text-slate-500 mt-1 text-base">Total Anggota</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-50">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h4 class="text-xl font-bold text-slate-800 m-0">Daftar Anggota</h4>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-100 text-slate-500">
                                <th class="pb-4 font-semibold px-4 w-20">Foto</th>
                                <th class="pb-4 font-semibold px-4">Nama</th>
                                <th class="pb-4 font-semibold px-4">Email</th>
                                <th class="pb-4 font-semibold px-4">Role</th>
                                <th class="pb-4 font-semibold pl-4 w-32">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            <?php if(!empty($users)){ foreach($users as $d){ ?>
                            <tr class="hover:bg-slate-50 transition-colors group">
                                <td class="py-4 px-4">
                                    <img src="https://cdn-icons-png.flaticon.com/512/3135/3135715.png" alt="Profile" class="w-10 h-10 rounded-full border border-slate-200 shadow-sm">
                                </td>
                                <td class="py-4 px-4 text-slate-800 font-semibold"><?= htmlspecialchars($d['nama']) ?></td>
                                <td class="py-4 px-4 text-slate-600"><?= htmlspecialchars($d['email']) ?></td>
                                <td class="py-4 px-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold <?= $d['role'] == 'admin' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-blue-100 text-blue-700 border-blue-200' ?> border">
                                        <?= ucfirst(htmlspecialchars($d['role'])) ?>
                                    </span>
                                </td>
                                <td class="py-4 pl-4">
                                    <div class="flex items-center gap-2">
                                        <button onclick="openModal('edit<?= $d['id'] ?>')" class="w-8 h-8 rounded-lg bg-yellow-100 text-yellow-600 hover:bg-yellow-500 hover:text-white flex items-center justify-center transition-colors" title="Edit">
                                            <i class="bi bi-pencil-fill"></i>
                                        </button>
                                        <a href="hapus_anggota.php?id=<?= $d['id'] ?>" class="w-8 h-8 rounded-lg bg-red-100 text-red-600 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors" onclick="return confirm('Yakin hapus data ini?')" title="Hapus">
                            ...
                            <?php }} else { ?>
                            <tr>
                                <td colspan="5" class="text-center text-slate-500 py-12 text-lg">
        ...
        <div class="relative bg-white rounded-3xl shadow-xl w-full max-w-md transform scale-95 opacity-0 transition-all duration-300 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h5 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <i class="bi bi-person-plus text-blue-600"></i> Tambah Anggota
                </h5>
            ...
            <form action="store_anggota.php" method="POST">
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Nama</label>
                        <input type="text" name="nama" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" placeholder="Nama lengkap" required>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Email</label>
                        <input type="email" name="email" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" placeholder="Alamat email" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Role</label>
                        <select name="role" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700 bg-white" required>
                    ...
                    <div>
                        ...
                        <input type="password" name="password" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" placeholder="Password akun" required>
                    </div>
                </div>
                ...
                <div class="px-6 py-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-md transition-colors">Simpan Anggota</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <!-- MODAL EDIT LOOP -->
    <?php if(!empty($users)){ foreach($users as $d){ ?>
    <div id="edit<?= $d['id'] ?>" class="fixed inset-0 z-[60] hidden flex-col items-center justify-center p-4 sm:p-0">
        <!-- Backdrop -->
        <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity opacity-0" onclick="closeModal('edit<?= $d['id'] ?>')"></div>
        
        <!-- Modal Content -->
        <div class="relative bg-white rounded-3xl shadow-xl w-full max-w-md transform scale-95 opacity-0 transition-all duration-300 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <h5 class="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <i class="bi bi-pencil-square text-yellow-500"></i> Edit Anggota
                </h5>
                <button onclick="closeModal('edit<?= $d['id'] ?>')" class="text-slate-400 hover:text-red-500 text-xl transition-colors">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>
            ...
            <form action="update_anggota.php" method="POST">
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Nama</label>
                        <input type="text" name="nama" value="<?= htmlspecialchars($d['nama']) ?>" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Email</label>
                        <input type="email" name="email" value="<?= htmlspecialchars($d['email']) ?>" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" required>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Password Baru <span class="font-normal text-slate-400">(Opsional)</span></label>
                        <input type="password" name="password" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700" placeholder="Kosongkan jika tidak diubah">
                    </div>
                </div>
                
                <div class="px-6 py-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                    <button type="button" onclick="closeModal('edit<?= $d['id'] ?>')" class="px-5 py-2.5 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-colors">Batal</button>
